<template>
    <div class="work-management">
        <div class="work-management__body">
            <!-- 리스트 -->
            <CommonContent :label="'안전교육'" @send-search="checkContentParams">
                <safetyEducationTable
                    :label="'안전교육'"
                    :menu-list="menuList"
                    :data="tableData"
                    :limit="limit"
...
import safetyEducationTable from "./Table.vue"
...
// 테이블 메뉴 리스트
const menuList: string[] = ["업체", "공종", "교육종류", "강사명", "교육일자", "교육시간"]
...
const getData = () => {
    ...
    let params = {
        ...
        education_start_date: contentParams.start_date !== "" ? contentParams.start_date : "1900.01.01",
        education_end_date: contentParams.end_date !== "" ? contentParams.end_date : dayjs(new Date(Date.now())).format("YYYY.MM.DD"),
    }
    params = Object.assign({}, params, headerParams)
    api.getEduList(params).then((res:any) => {
        try {
