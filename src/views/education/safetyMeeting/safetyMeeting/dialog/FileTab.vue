<template>
    <div class="tab-image">
        <div class="tab-image__header">
            <button class="tab-image__header__button" :class="{ active: isVisibleImage }" @click="showImage">회의사진</button>
            <button class="tab-image__header__button" :class="{ active: isVisibleTable }" @click="showTable">회의참석인원</button>
        </div>
        <div v-if="isVisibleImage" class="tab-image__body">
            <div v-if="!photos.length" class="tab-image__body__image-box">
                ...
                <button v-if="!disabled" class="button" @click.prevent="openUpload">
        ...
        <div v-if="isVisibleTable" class="tab-image__body" style="display: flex; flex-direction: column">
            <BasicButton v-if="!disabled" style="margin: -16px 0 -20px 4px" :label="'참석인원 추가'" :theme="'fill'" @click="openSelectWorkerDialog" />
            <BasicButton v-else style="margin: -16px 0 -20px 4px" :label="'참석인원 추가'" :theme="'cancel'" />
            <!-- 참석자 테이블 -->
            <div class="tab-image__body__table">
                ...
                <div class="header">
                    ...
                    <div class="header__menu" :style="{ width: `${width}%` }"></div>
                </div>
                <!-- 테이블 리스트 -->
                <div class="body">
...
const emit = defineEmits(["send-event", "delete-file", "open-select", "send-data"])
...
const deleteImage = (file: any, index: number) => {
    if (photos.value.length === 0) {
        ...
        files.value.forEach((item: any, itemInex: number) => {
            if (item.name === file.name) {
                files.value.splice(itemInex, 1)
            }
        })
    } else {
