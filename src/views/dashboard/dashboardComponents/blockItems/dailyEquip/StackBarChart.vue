...
watch(data, ()=>{
    if(instanceChart) {
        instanceChart.data.datasets[0].data = data.value.map( (row:any) => row.com_cnt)
        instanceChart.data.datasets[1].data = data.value.map( (row:any) => row.standby_cnt)
        ...
        datasets: [
            {
                type: 'bar',
                label: '점검',
                data: data.value && data.value.map( (row:any) => row.com_cnt),
                backgroundColor: '#78c8ff',
            },
            {
                type: 'bar',
                label: '미점검',
                data: data.value && data.value.map( (row:any) => row.standby_cnt),
