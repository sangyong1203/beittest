    ...
    <div class="dialog">
        <div class="dialog-layout">
            <div class="dialog-layout__header">
                <span v-if="!isUpdate" class="dialog-layout__header__title">{{ title }} 등록</span>
                <span v-if="isUpdate" class="dialog-layout__header__title">{{ title }} 상세</span>
            ...
            <div v-if="isApproval" class="dialog-layout__footer">
                <BasicButton :label="'인쇄하기'" :theme="'outer'" @click="onPrint"></BasicButton>
            </div>
            <div v-else class="dialog-layout__footer">
                <BasicButton class="print-button" v-if="isUpdate && !isVisibleSave" :label="'인쇄하기'" :theme="'outer'" @click="onPrint"></BasicButton>
                <BasicButton v-if="!isUpdate" :label="'취소'" :theme="'cancel'" @click="closeDialog" />
                <BasicButton v-if="!isUpdate" :label="'등록'" :theme="'fill'" @click="onSubmit" />
                <BasicButton v-if="isUpdate && !isVisibleSave" :label="'삭제'" :theme="'cancel'" @click="openPopup" />
                <BasicButton v-if="isUpdate && !isVisibleSave" :label="'수정'" :theme="'fill'" @click="onUpdate" />
                <BasicButton v-if="isVisibleSave" :label="'취소'" :theme="'cancel'" @click="closeDialog" />
...
const { title, isApproval, isUpdate } = toRefs(props)

// 다이얼로그 닫기 외 버튼 기능 함수
const emit = defineEmits(['close-dialog', 'submit', 'update', 'delete', 'save', 'close-findDialog', 'print'])
