<template>
    <div class="date-field">
        <div class="date-field__label-box" :style="{ width: labelWidth ? `${labelWidth}` : `120px` }">
            <span class="date-field__label-box__label">{{ label }}</span>
            <span class="date-field__label-box__required-mark" :class="{ active: required }">*</span>
        </div>
        <input
            :value="modelValue"
            @input="updateDate"
            type="text"
            :disabled="disabled"
            :placeHolder="placeholder ? placeholder : checkPlaceholder(label)"
            maxlength="10"
            class="date-field__input"
            :class="{ disabled: disabled, validation: validation }"
            :style="{ width: inputWidth ? `${inputWidth}` : `230px` }"
        />
        <span v-if="validation" class="date-field__validation">{{ checkValidation(label) }}</span>
    </div>
</template>
...
const { label, required, disabled, validation, placeholder, labelWidth, modelValue } = toRefs(props)
...
const updateDate = (event: any) => {
    ...
    inputValue.value = originValue.replace(/(\d{4})(\d{2})(\d{2})/g, "$1.$2.$3")
